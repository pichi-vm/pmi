...
The `cca` platform is the Arm CCA launch path. A VMM targeting `cca` reads
the `.pmi.cca` PE section (non-loaded; `IMAGE_SCN_MEM_DISCARDABLE`). If the
section is absent, the image does not support `cca`.

The `cca` platform is independent of [`vm`](vm.md) and [`sev`](sev.md). It is
